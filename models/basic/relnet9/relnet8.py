...
train_y = torch.stack([rrn_utils.encode_input(p) for p in train_outputs]).cuda(hp['device'])
...
for k in other_inputs:
    ...
    other_y[k] = torch.stack([rrn_utils.encode_input(p) for p in other_outputs[k]]).cuda(hp['device'])
