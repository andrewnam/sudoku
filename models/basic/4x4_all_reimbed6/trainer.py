...
Reimbed 6: Output is changed to predict a zero as well (which should never actually happen).
    Output_dim is changed to accomodate.
    (Y vectors are encoded using encode_input rather than encode_output)
"""
...
def train_rrn(hyperparameters: dict,
    ...
    train_y = torch.stack([rrn_utils.encode_input(p) for p in train_outputs]).cuda(device)
    ...
    for k in other_inputs:
        ...
        other_y[k] = torch.stack([rrn_utils.encode_input(p) for p in other_outputs[k]]).cuda(device)
