...
class Datasets:
    ...
    def values(self):
        return [self.dataset[k] for k in self.dataset_keys]

    def items(self):
        return zip(self.keys(), self.values())

    def data_keys(self):
        return utils.flatten([self.datasets[k].keys() for k in self.keys()])
    ...
    def save(self, filename):
        with open(filename, 'wb') as f:
            pickle.dump({'dataset_keys': self.keys(),
                         'datasets': {
                             name: {
                                 'data': dst.data,
    ...
    def load(filename):
        ...
        datasets.dataset_keys = data['dataset_keys']

        for name in data['datasets']:
            dst = Dataset(data['datasets'][name]['data'])
            dst.input_order = data['datasets'][name]['input_order']
            datasets.datasets[name] = dst
