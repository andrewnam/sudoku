...
Relnet6: Batchsize = 1200 (no batch, basically).
        Reduced number of layers by 1 for each MLP, including relational layer
Relnet7: Reduce number of layers by 1 for each MLP again.
        This means the network has no nonlinearities.
Embnet1: 1. embed dim=3
            last layers of network :
                linear to dim=3
                linear to dim=max_digit, call this decoder
                softmax
            Secondary network: input -> embedding -> decoder -> softmax
         2. losses
                softmax-cross entropy on the task
                softmax-cross entropy on autoencoder
"""
...
class RelNet(nn.Module):
    ...
    def __init__(self, dim_x, dim_y, embed_size, hidden_layer_size):
        ...
        self.embed_mlp = MLP([self.embed_size + self.max_digit, self.hidden_layer_size])
    ...
    def forward(self, x_grid_form, x_prob_form, iters):
        ...
        task_outputs = torch.empty(iters, batch_size, num_nodes, self.max_digit).cuda(device)

        embedding = self.encoder(x_grid_form)
        ...
        return {'decoded': decoded, 'task': task_outputs}
...
def get_performance(model, x_grid, x_prob, y, num_iters=32, no_grad=True):
    ...
    def run():
        ...
        decode_predictions = predictions['decoded']
        task_predictions = predictions['task']


        task_loss = sum([F.cross_entropy(p.permute(0, 2, 1), y) for p in task_predictions])
        task_accuracies = torch.sum(torch.argmax(task_predictions, dim=3) == y, dim=2).permute(1, 0)
        return task_loss, task_accuracies.cpu().data.numpy()
