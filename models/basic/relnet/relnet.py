...
train_x = torch.stack([rrn_utils.encode_input(p) for p in train_inputs]).cuda(hp['device'])
...
for k in other_inputs:
    other_x[k] = torch.stack([rrn_utils.encode_input(p) for p in other_inputs[k]]).cuda(hp['device'])
    # other_x[k] = utils.one_hot_encode(other_x[k])
...
def closure():
    ...
    for i in tqdm(range(0, len(train_x), hp['batch_size']), leave=False):
        x_batch = train_x[shuffle_indices[i:i + hp['batch_size']]]
